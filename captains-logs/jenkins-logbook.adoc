...
* Tried the method described in the voxxed-lu demo.
  ...
  As I enabled OAuth, I should use the authentication method described in the plugin notes (use a github token to authenticate).
  This doesn't work. +
...
* I will try an alternative method (less automatic) to setup an agent: the SSL method.
  I plan to use the pre-packaged ssh-slave. I will have to solve the scripting of the key generation problem and the configuration of the plugin.
